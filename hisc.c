...
void draw_hisc_post(Thisc *table, int x, int y) {
	//print(x, y, "%s", table->name);
	//print(x+140, y, "%d", table->score);
	char str[64];
	if(table->score > 0 ){

        sprintf(str, "%s", table->name);
        print(str, x, y);

        //sprintf(str, "%s  %d", table->name, table->score );
        //print(str, x, y);

        //strcpy(str, table->score);
        //print(str, x + 140, y);
	}
}
*/
...
void draw_hisc_table(Thisc *table, int x, int y) {
	...
	for( i=0; i < MAX_SCORES; i++){
		draw_hisc_post(&table[i], x, y + 11 * i);
	}
}
*/
